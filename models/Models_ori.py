...
class DPCD(nn.Module):
    ...
    def forward(self, t1, t2, log=False, img_name=None, scale_ratio = None):
        ...
        # tf1, tf2, hx2_ffted = self.SAF(t1, t2)

        # t1, t2 = self.mask_query(t1, t2, maskA, maskB)
        # # t1_1, t2_1 = torch.chunk(self.en_block1(torch.concat((t1, t2), dim=1)), 2, dim=1)
        # t2_1 = self.en_block1((t1 - t2))
        # ext1_1 = self.ex_en_block1(tf1)
        # ext2_1 = self.ex_en_block1(tf2)
        # t1_1, t2_1 = self.fuse_conv[0](torch.cat((t2_1, ext1_1), dim=1)), self.fuse_conv[0](torch.cat((t2_1, ext2_1), dim=1))
        #
        # # t1_2, t2_2 = torch.chunk(self.en_block2(torch.concat((t1_1, t2_1), dim=1)), 2, dim=1)
        # t2_2 = self.en_block2((t1_1 - t2_1))
        # ext1_2 = self.ex_en_block2(ext1_1)
        # ext2_2 = self.ex_en_block2(ext2_1)
        # t1_2, t2_2 = self.fuse_conv[1](torch.cat((t2_2, ext1_2), dim=1)), self.fuse_conv[1](torch.cat((t2_2, ext2_2), dim=1))
        #
        # # t1_3, t2_3 = torch.chunk(self.en_block3(torch.concat((t1_2, t2_2), dim=1)), 2, dim=1)
        # t2_3 = self.en_block3((t1_2 - t2_2))
        # ext1_3 = self.ex_en_block3(ext1_2)
        # ext2_3 = self.ex_en_block3(ext2_2)
        # t1_3, t2_3 = self.fuse_conv[2](torch.cat((t2_3, ext1_3), dim=1)), self.fuse_conv[2](torch.cat((t2_3, ext2_3), dim=1))
        # t1_3, t2_3 = self.channel_exchange4(t1_3, t2_3)
        #
        # # t1_4, t2_4 = torch.chunk(self.en_block4(torch.concat((t1_3, t2_3), dim=1)), 2, dim=1)
        # t2_4 = self.en_block4((t1_3-t2_3))
        # ext1_4 = self.ex_en_block4(ext1_3)
        # ext2_4 = self.ex_en_block4(ext2_3)
        # t1_4, t2_4 = self.fuse_conv[3](torch.cat((t2_4, ext1_4), dim=1)), self.fuse_conv[3](torch.cat((t2_4, ext2_4), dim=1))
        # # t1_4, t2_4 = self.channel_exchange4(t1_4, t2_4)
        #
        # # t1_5, t2_5 = torch.chunk(self.en_block5(torch.concat((t1_4, t2_4), dim=1)), 2, dim=1)
        # t2_5 = self.en_block5((t1_4 - t2_4))
        # ext1_5 = self.ex_en_block5(ext1_4)
        # ext2_5 = self.ex_en_block5(ext2_4)
        # t1_5, t2_5 = self.fuse_conv[4](torch.cat((t2_5, ext1_5), dim=1)), self.fuse_conv[4](torch.cat((t2_5, ext2_5), dim=1))
